...
{***********************************************************************}
BEGIN	{ main program }
	MoreMasters; { Allocates another block of master pointers in current heap, II-31 }
	MoreMasters; { Allocates another block of master pointers in current heap, II-31 }
	InitGraf(@thePort); { Initializes QuickDraw, I-162 }
	InitFonts; { Initializes the Font Manager, I-222 }
	InitWindows; { Initializes the Window Manager, I-281 }
	InitMenus; { Initializes the Menu Manager, I-351 }
	InitDialogs(NIL); { Initializes the Dialog Manager, I-411 }
	MainWind:=GetNewWindow(304,NIL,Pointer(-1)); {Creates "Title Screen" window specified by paramenters, 304 = resource ID of window template, see NewRescr.r 3440-3447 }
	SetPort(MainWind); {Makes specific grafPort the current port }
	RandSeed:=TickCount; { Returns current # of ticks (1/60th of second) since system last started up }
	REPEAT
		TwoPFlag:=TitlePage; { See Titles.p Function TitlePage; -> TitlePage := (WhichControl = ContHndl); | returns Boolean to TwoPFlag }
		EndDummy:=InitGame;
